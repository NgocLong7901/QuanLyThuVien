...
//    --- 1. Bạn đọc chưa trả sách
    public ResultSet chuatrasach () {
        String sql = "SELECT a.maPhieuMuon ,b.maDocGia, b.tenDocGia\n" +
                    "FROM dbo.PhieuMuon AS a, dbo.DocGia AS b\n" +
                    "WHERE a.maDocGia = b.maDocGia AND a.trangThai = N'Chưa trả'";
        try {
...
//    --- 2. Bạn đọc mượn quá hạn
    public ResultSet quahantra () {
        String sql = "SELECT c.tenDocGia, a.ngayMuon, a.soNgayMuon, b.ngayThucTra, ((DAY(b.ngayThucTra) - DAY(a.ngayMuon)) - a.soNgayMuon) AS 'Số ngày quá hạn'\n" +
                    "FROM dbo.PhieuMuon AS a, dbo.ChiTietPhieuMuon AS b, dbo.DocGia AS c\n" +
                    "WHERE a.maPhieuMuon = b.maPhieuMuon AND a.maDocGia = c.maDocGia\n" +
...
//    --- 3. Bạn đọc mượn nhiều nhất
    public ResultSet muonnhiunhat () {
        String sql = "SELECT DISTINCT TOP 10 COUNT(a.maDocGia) AS 'Số lần mượn', b.tenDocGia\n" +
                    "FROM dbo.PhieuMuon AS a, dbo.DocGia AS b\n" +
                    "WHERE a.maDocGia = b.maDocGia\n" +
                    "GROUP BY b.tenDocGia\n" +
...
//    --- 4. Sách được mượn nhiều nhất theo ngày
    public ResultSet sachdocnhiunhat () {
        String sql = "SELECT DISTINCT TOP 100 COUNT(a.maSach) AS 'Số lần mượn trong ngày', a.maSach, b.tenSach, c.ngayMuon\n" +
