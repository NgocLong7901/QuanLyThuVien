...
public class DocGia_Dao_implement implements DocGia_Dao{
    ...
    public int addDocgia(Docgia dg) {
        String sql = "insert into DocGia(maDocGia, matKhau, tenDocGia, ngaySinh, gioiTinh, email, sdt)"
                + "values (?,?,?,?,?,?,?)";
        try {
            PreparedStatement stm = KetNoiSQL.getConnection().prepareStatement(sql);
    ...
    public int updateDocgia(Docgia dg) {
        String sql = "update DocGia set tenDocGia =?, ngaySinh = ?, gioiTinh = ?, email = ?, sdt = ? where maDocGia = ?";
        try {
            PreparedStatement stm = KetNoiSQL.getConnection().prepareStatement(sql);
    ...
    public Docgia searchDg(String madocgia) {
     ...
     public Docgia searchtendg(String tendocgia) {
        String sql = "select * from DocGia where tenDocGia = ?";
        try {
            PreparedStatement stm = KetNoiSQL.getConnection().prepareStatement(sql);
            ResultSet allDocgia = null;
            stm.setString(1, tendocgia);
            allDocgia = stm.executeQuery();
    ...
    public int getOneDocgia(String madocgia) {
        String sql = "select [status] from DocGia where maDocGia = ?";
        try {
            PreparedStatement stm = KetNoiSQL.getConnection().prepareStatement(sql);
    ...
    public int unlock(String madocgia) {
        String sql = "update DocGia set status = 1 where maDocGia = ?";
        try {
            PreparedStatement stm = KetNoiSQL.getConnection().prepareStatement(sql);
            stm.setString(1, madocgia);
            return stm.executeUpdate();
